...
// 获取项目前缀，用于表名区分
const PROJECT_PREFIX = 'tower_jump'; // 硬编码以修复表命名问题
...
async function validateProject(projectId) {
  try {
    ...
    if (project.length === 0) {
      // 自动创建项目注册
      console.log(`[项目验证] 项目 "${projectId}" 未注册，自动创建项目注册`);
      const feedbackTableName = `${projectId}_feedback`;
      const newProject = await sql`
        INSERT INTO game_projects (project_id, project_name, feedback_table_name, is_active, created_at)
        VALUES (${projectId}, ${projectId}, ${feedbackTableName}, true, CURRENT_TIMESTAMP)
        RETURNING id, is_active
      `;
      console.log(`[项目验证] 项目 "${projectId}" 注册成功，反馈表: ${feedbackTableName}`);
      return newProject[0];
    }
...
async function validateAdminPermission(adminId, projectId) {
  try {
    ...
    // 如果项目注册表不存在或JOIN失败，只检查管理员是否存在
    if (error.message.includes('relation "game_projects" does not exist') || 
        error.message.includes('column gp.is_active does not exist')) {
      const admin = await sql`
...
const initializeAdmin = async () => {
  try {
    ...
    try {
      ...
      console.log(`[管理员初始化] 项目注册失败，但继续创建管理员: ${error.message}`);
    }
    ...
    if (existingAdmin.length === 0) {
      ...
      try {
        ...
        console.error('插入管理员账户时出错:', insertError);
      }
    } else {
